 ...
 public synchronized String northbound_arrival()
  {
   switch( lock )
    {
     case SOUTH:
         // change the lights if the wrong direction has GREEN
         if( northLight.equals(GREEN) )
     ...
     case NONE:
         ...
         if( !northLight.equals(southLight) )
           displayLights( GREEN, GREEN );
     ...
     case NORTH:
         // if we already have the number of new arrivals allowed to enter
         if( arrivals >= LIMIT )
          {
           // change the lights if necessary
           if( !northLight.equals(southLight) )
             displayLights( RED, RED );
 ...
 public synchronized String southbound_arrival()
  {
   switch( lock )
    {
     case NORTH:
         // change the lights if wrong direction has GREEN
         if( southLight.equals(GREEN) )
           displayLights( GREEN, RED );
     ...
     case SOUTH:
         ...
         // otherwise, change the lights if the wrong direction has RED
         if( southLight.equals(RED) )
           displayLights( RED, GREEN );

         // increment the number of arrivals if the other queue is occupied
         if( northQueue.occupied() )
           arrivals++ ;
 ...
 public synchronized void northbound_departure( int car_identification )
  {
   ...
   if( entered == 0 )
    {
     if( southQueue.occupied() )
      /* if southbound traffic is waiting, give South the lock and signal the South threads */
      {
       lock = SOUTH ;
       southQueue.cSignal() ;
      }
 ...
 public synchronized void southbound_departure( int car_identification )
  {
   ...
   if( entered == 0 )
    {
     if( northQueue.occupied() )
      /* if northbound traffic is waiting, give North the lock and signal the North threads */
      {
       lock = NORTH ;
       northQueue.cSignal() ;
      }
